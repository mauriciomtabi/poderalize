...
export const AddLeadToFunnelDialog = ({
  open,
  onOpenChange,
  stageId
}: AddLeadToFunnelDialogProps) => {
  const {
    state,
    updateFunnel
  } = useCRM();
  ...
  useEffect(() => {
    if (open) {
      requestAnimationFrame(() => {
        ...
        existingScrollRef.current?.scrollTo({
          top: 0
        });
      });
    }
  ...
  useEffect(() => {
    requestAnimationFrame(() => {
      if (selectedTab === 'new') newScrollRef.current?.scrollTo({
        top: 0
      });
      if (selectedTab === 'existing') existingScrollRef.current?.scrollTo({
        top: 0
      });
    });
  }, [selectedTab]);
  const filteredExistingLeads = existingLeads.filter(lead => lead.nome.toLowerCase().includes(searchTerm.toLowerCase()) || lead.empresa.toLowerCase().includes(searchTerm.toLowerCase()) || lead.email.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  const handleAddExistingLead = (lead: Lead) => {
    ...
    updateFunnel(state.currentFunnel.id, {
