...
import { usePlanos, TipoPlano, CreatePlanoData } from "@/hooks/usePlanos";

interface CreatePlanoDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  defaultTipo?: TipoPlano;
}
...
export function CreatePlanoDialog({ open, onOpenChange, defaultTipo }: CreatePlanoDialogProps) {
  const { createPlano } = usePlanos();
  const [isSubmitting, setIsSubmitting] = useState(false);

  const [formData, setFormData] = useState<CreatePlanoData>({
    nome: '',
    tipo: defaultTipo || 'social_media',
    descricao: '',
    ativo: true,
    configuracoes: {},
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await createPlano(formData);
    if (success) {
      onOpenChange(false);
      setFormData({
        nome: '',
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Defina as características do plano de serviço
            </DialogDescription>
          </DialogHeader>
