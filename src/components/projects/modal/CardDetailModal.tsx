...
export const CardDetailModal = ({
  ...
  const completedTasks = latestCard.checklists.reduce((acc, checklist) => acc + checklist.items.filter(item => item.completed).length, 0);
  const totalTasks = latestCard.checklists.reduce((acc, checklist) => acc + checklist.items.length, 0);
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden p-0" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1 mr-4">
                    {isEditingTitle ? <Input ref={titleInputRef} value={title} onChange={e => setTitle(e.target.value)} onBlur={handleSaveTitle} onKeyDown={e => handleKeyDown(e, handleSaveTitle)} className="text-lg font-semibold border-none p-0 h-auto focus-visible:ring-0" /> : <h2 className="text-lg font-semibold cursor-pointer hover:bg-muted/50 rounded p-1 -m-1 transition-colors" onClick={() => setIsEditingTitle(true)}>
                        {latestCard.title}
                      </h2>}
                ...
                {latestCard.labels.length > 0 && <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {latestCard.labels.map(label => <Badge key={label.id} style={{
          ...
          <div className="w-64 border-l bg-muted/30 flex-shrink-0">
            <ScrollArea className="h-[80vh] p-4">
              <div className="space-y-4">
                ...
                {/* Current assignees */}
                {latestCard.assignees.length > 0 && <div>
                    <h4 className="text-sm font-medium mb-2">Membros</h4>
                    <div className="flex flex-wrap gap-2">
                      {latestCard.assignees.map(member => <div key={member.id} className="flex items-center gap-2 bg-background rounded p-2">
        ...
        {/* Dialogs */}
        <MemberPicker isOpen={showMemberPicker} onClose={() => setShowMemberPicker(false)} availableMembers={availableMembers} selectedMembers={latestCard.assignees} onMembersChange={handleMembersChange} />

        <LabelPicker isOpen={showLabelPicker} onClose={() => setShowLabelPicker(false)} availableLabels={availableLabels} selectedLabels={latestCard.labels} onLabelsChange={handleLabelsChange} />

        <DueDatePicker isOpen={showDueDatePicker} onClose={() => setShowDueDatePicker(false)} currentDate={latestCard.dueDate} onDateChange={handleDueDateChange} />
