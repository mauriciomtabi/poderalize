...
export const ChecklistTemplateManager = ({
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingTemplate) {
        await updateTemplate(editingTemplate.id, formData.title, validItems.map((text, i) => {
        ...
        }), formData.description || undefined, formData.isGlobal);
      } else {
        await createTemplate(formData.title, validItems, formData.description || undefined, formData.isGlobal, boardId);
      }
      setIsCreating(false);
      setEditingTemplate(null);
      setFormData({
  ...
  return <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh]">
        ...
        {!isCreating ? <div className="space-y-4">
          ...
          }))} placeholder="Descreva o propósito deste template..." rows={2} />
            </div>

            <div className="flex items-center space-x-2">
              <Switch id="global" checked={formData.isGlobal} onCheckedChange={checked => setFormData(prev => ({
            ...prev,
            isGlobal: checked
          }))} />
