      ...
      <DialogContent className="max-w-2xl max-h-[80vh]">
        ...
        <div className="space-y-4">
          <div className="flex gap-2">
            <Button variant="outline" className="flex-1" onClick={handleCreateEmpty}>
              Criar Lista Vazia
            </Button>
          </div>
              ...
              <div className="space-y-2 pr-4">
                {filteredTemplates.length === 0 ? <div className="text-center py-12">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      ...
                      {template.description && <p className="text-sm text-muted-foreground mb-3">
                          {template.description}
                        </p>}

                      {template.items.length > 0 && <div className="space-y-1">
                          <p className="text-xs font-medium text-muted-foreground">
                            Preview dos itens:
                          </p>
                          {template.items.slice(0, 3).map((item, index) => <div key={item.id} className="text-xs text-muted-foreground flex items-center gap-2">
                              <CheckSquare className="w-3 h-3" />
                              {item.text}
                            </div>)}
                          {template.items.length > 3 && <p className="text-xs text-muted-foreground italic">
